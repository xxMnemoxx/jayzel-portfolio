...
export default function ScrollProgress() {
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      rafId = requestAnimationFrame(() => setProgress(clampedProgress));
    };
  ...
  return (
    <div className="pointer-events-none fixed right-3 top-1/2 z-40 -translate-y-1/2 transition-all duration-300 sm:right-5">
      <div className="flex flex-col items-center gap-2">
        <span className="pixel-font min-w-11 text-center text-[7px] uppercase tracking-[0.2em] text-cyan-200/90 tabular-nums">
          {Math.round(progress)}%
        </span>

        <div className="flex h-32 w-2 items-end overflow-hidden rounded-[0.15rem] border border-amber-200/20 bg-slate-900/80 sm:h-36">
          <div
            className="w-full rounded-[0.1rem] bg-linear-to-t from-amber-300 via-yellow-400 to-cyan-300 transition-[height] duration-300 ease-out"
